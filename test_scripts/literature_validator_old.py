...
def gather_literature(drug: str,
    ...
    if pdb_id:
        ...
        # ── Step 2: Get primary citation ───────────────────
        citation_pmid = structure_info.get("citation_pmid")

        if citation_pmid:
            # RCSB gave us a valid, year-validated PMID
            if citation_pmid not in seen:
                all_pmids.append(citation_pmid)
                seen.add(citation_pmid)
                print(f"  ✓ Primary citation (RCSB validated): "
                      f"PMID {citation_pmid}")
        else:
            # RCSB citation was invalid or missing —
            # search PubMed directly using the PDB [si] field
            ...
            if fallback_pmid and fallback_pmid not in seen:
                ...
                print(f"  ✓ Primary citation (PubMed [si]): "
                      f"PMID {fallback_pmid}")
    ...
    # Mark first paper as primary citation
    if papers:
        papers[0]["is_primary"] = True
